<template>
  <div class="flex flex-col gap-8 justify-between w-full content-container pt-16 pb-12 max-w-7xl">
    ...
    <div class="flex w-full items-center gap-12">
      ...
      <div class="flex flex-col  gap-1">
        <p class="h6 font-medium">{{ firstName }} {{ lastName }}</p>
        <p class="base">{{ bio }}</p>
      </div>
    </div>
    <!--    names-->
    <div class="flex flex-col lg:flex-row gap-4 lg:gap-8 items-center w-full">
      <tok-input :disabled="!isOwner" id="userName" v-model="userName" label="Username" placeholder=" "/>
      <tok-text-area  :disabled="!isOwner" id="bio" v-model="bio" label="Bio" placeholder=" "/>
    </div>

    <div class="flex flex-col lg:flex-row gap-4 lg:gap-8 items-center w-full">
      <tok-input :disabled="!isOwner" id="firstName" v-model="firstName" label="First Name" placeholder=" "/>
      <tok-input :disabled="!isOwner" id="lastName" v-model="lastName" label="Last Name" placeholder=" "/>
    </div>
    ...
    <button v-if="isOwner && !state.loading" class="primary-btn w-44 self-center" @click="handleSave">Save Changes</button>
